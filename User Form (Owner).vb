Public Class frm_user
    ...
    Public Sub ViewSelectedlvDetails(ByVal ID As String)
        Dim dt As New DataTable
        Dim ctr As Integer

        dt = ExecuteQuery("SELECT * FROM [User] WHERE [ID] = '" & ID & "'")
        If dt.Rows.Count > 0 Then
    ...
    Public Sub Edit()
        Try
            Dim dt As New DataTable
            dt = ExecuteQuery("UPDATE [User] SET [Username] = '" & txtbox_uname.Text.Trim & "', [Password] = '" & txtbox_pass.Text.Trim & "', [Position] = '" & txtbox_position.Text.Trim & "' WHERE [ID] = '" & txtbox_UserID.Text & "'")

            lvUL.Items.Clear()
            Call Fill_User()

            MessageBox.Show(txtbox_uname.Text)
    ...
    Private Sub txtbox_UserID_TextChanged(sender As Object, e As EventArgs) Handles txtbox_UserID.TextChanged
